...
| M17 Easy going | Step-up onto ledges up to 0.6 m (with the view easing up), walkable slopes to about 63 degrees and slow scrambling to about 72 degrees while pushing; terrain v2 for new worlds (soft terraces, gently rumpled open ground; older saves keep theirs); brighter cave ambient, sky light fading over 20 m under the surface and spreading further from openings; cave-mouth tunnels kept 8 m under lakes | `tests/physics.test.ts`, `tests/terrain.test.ts`, `tests/caves.test.ts`, `scripts/probe-walk.mjs` (stalls on seed 1337: 10 before, 3 after) |
